...
\[\tensel{A}_{i_1 \ldots i_d} = \underbrace{G_1[i_1]}_{1 \times \rank} \underbrace{G_2[i_2]}_{\rank \times \rank} \ldots \underbrace{G_d[i_d]}_{\rank \times 1}\]
...
\begin{itemize}
  ...
  \item Maximize ELBO with respect to TT-cores of $\mu$, Kronecker factors of 
    $\Sigma$, and kernel parameters using stochastic optimization
\end{itemize}

\section*{\LARGE \color{NavyBlue} Properties}
    
\begin{itemize}
  \item Linear complexity $\bigO(n D m^{1 / D} r^2 + D m^{1 / D} r^3 + D m^{3 / D})$ w.r.t. the dataset size $n$ and dimensionality $D$. 
...
\begin{tabular}{cc}
  \includegraphics[width=20cm]{pics/embed/embedding_2.eps} &
    \includegraphics[width=35cm, trim={2cm 19.5cm 0cm 0}, clip]{pics/tikz/pic.pdf}
    \\
  Embedding learned by TT-GP with a deep kernel on {\it digits}&
  \hspace{-20cm}Deep Kernel GP model 
\end{tabular}
}
\end{center}
