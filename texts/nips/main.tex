...
%\parskip .35pc  % original size - .5pc
...
\begin{abstract}
  We propose an approximate inference method for Gaussian Processes (GPs) that
  scales to large datasets and input dimensions.
  We build on top of previous results on scalable GPs including stochastic
  variational inference based on inducing inputs, kernel interpolation, and
  structure exploiting algebra. The key idea of the proposed method is to utilize
  the Tensor Train decomposition for variational parameters, which allows us to train
  GP with billions of inducing inputs and to achieve state-of-the-art results
  on several benchmark datasets. Further, our approach allows us to train kernels based on
