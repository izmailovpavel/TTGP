...
def get_data():
    ...
    W_te_tf = inputs.interpolate_on_batch(x_te)
    y_te = make_tensor(y_te, 'y_te')
    return x_tr, y_tr, x_te, y_te, W, W_te, inputs, W_init, y_init, W_te_tf
...
with tf.Graph().as_default():
    ...
    FLAGS = process_flags()
    x_tr, y_tr, x_te, y_te, W, W_te, inputs, W_init, y_init, W_te_tf = get_data()
    ...
    with tf.Session() as sess:
        ...
        #batch_elbo = 0
        W_te_value = sess.run(W_te.tt_cores)
        W_te_tf_value = sess.run(W_te_tf.tt_cores)
        print([(core_1.shape, core_2.shape, np.max(np.abs(core_1 - core_2))) for core_1, core_2 in zip(W_te_value, W_te_tf_value)])
        #print(list(zip(W_te_value, W_te_tf_value)))
        exit(0)
        if FLAGS.stoch:
